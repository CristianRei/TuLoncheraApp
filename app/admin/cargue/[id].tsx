...
export default function DetalleCargue() {
  ...
  function abrirReducir(linea: CargueLinea) {
    setLineaEnEdicion(linea);
    setModo('reducir');
    setValorTexto(String(linea.cantidadPlaneada));
  }

  function abrirResolver(linea: CargueLinea) {
    setLineaEnEdicion(linea);
    setModo('resolver');
  ...
  async function confirmarModal() {
    ...
    try {
      ...
      if (modo === 'reducir') {
        await reducirLineaCargue(db, { lineaId: lineaEnEdicion.id, nuevaCantidad: valor });
      } else {
        await resolverLineaEnRevision(
          db,
    ...
    <View style={styles.contenedor}>
      <View style={[styles.encabezado, { paddingTop: insets.top + 20 }]}>
        <ContenedorAncho anchoMaximo={720} style={styles.encabezadoContenido}>
          ...
          <Text style={styles.titulo}>Detalle del cargue</Text>
        </ContenedorAncho>
      </View>
      ...
      {cargando ? (
        ...
        <ContenedorAncho anchoMaximo={720} llenarAlto>
          <View style={styles.resumen}>
            ...
            <Text style={styles.resumenDetalle}>Estado: {cargue.estado}</Text>
          </View>
